...
function validateRawDataFile(raw_data, control, publication_idx, study_idx, dataset_idx) {
    ...
    if (invalid_congruency_vals.length > 0) {
        alert_message = `The "congruency" column contains invalid values: ${invalid_congruency_vals.slice(0, 5).join(', ')}. It should only contain "congruent", "incongruent", or "neutral", or "NA".`;
        displayValidationError('raw_data_file', alert_message);
        return false;
    }
